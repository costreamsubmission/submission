...
def plot(results):
    ...
    order = ["throughput", "e2e-latency", "proc-latency"]

    color1 = plt.rcParams['axes.prop_cycle'].by_key()['color'][0]
    color2 = plt.rcParams['axes.prop_cycle'].by_key()['color'][3]
    ...
    for i, (metric, dff) in enumerate(df.groupby("metric")):
        if metric == "throughput":
            ...
            for i, m in enumerate(["Q50", "Q95"]):
                ...
                dfff.plot(kind='bar', ax=axs[i], legend=None,  edgecolor='black',  width=0.9)
                """
